...
for row in range(6):
    for col in range(6):
        # FIX: Reduced width to 2, height to 1, and font to 20 to fit the screen
...
def check_guess(event=None): # Added 'event' parameter so the Enter key can trigger this
    global attempts
...
# FIX: Bind the 'Enter/Return' key to the check_guess function
window.bind('<Return>', check_guess)

# create a button to submit the guess (Now visible since grid is smaller!)
